local path_sep = require("reppuccin").path_sep
...
function M.compile()
	local theme = require("reppuccin.lib.mapper").apply()
	local lines = {
		[[
...
local h = vim.api.nvim_set_hl]],
	...
	if vim.g.reppuccin_debug then
		local deb = io.open(O.compile_path .. path_sep .. "reppuccin.lua", "wb")
		if deb then
	...
	if not f then
		local err_path = (path_sep == "/" and "/tmp" or os.getenv "TMP") .. "/reppuccin_error.lua"
		print(string.format(
			[[Reppuccin (error): Most likely some mistake made in your reppuccin config
...
Below is the error message that we captured:
	...
	local file = assert(
		io.open(O.compile_path .. path_sep .. "reppuccin", "wb"),
		"Permission denied while writing compiled file to " .. O.compile_path .. path_sep .. "reppuccin"
	)
	file:write(f())
	file:close()
end
